...
import type { StoryBeatMeta, StoryNarrativeBeatDisplay } from './storyNarrativeBridge'
...
/**
 * Right-side story column — flex sibling in AppShell (same layout model as
 * ZoneKPIOverlayPanel on the left). Shrinks the viewport instead of overlapping it.
 */
export default function StoryNarrativeRail({
  beat,
  index,
  total,
  color,
  ...
  if (collapsed) {
    return (
      <aside
        className="flex flex-col items-center shrink-0 w-11 h-full border-l border-white/10 bg-gray-950/30 backdrop-blur-xl z-10"
          ...
          {beatMeta.map((b, i) => (
            <button
              ...
              style={{
                width: i === index ? 8 : 6,
                height: i === index ? 8 : 6,
                backgroundColor: i === index ? (RUNG_COLOR[b.rung] ?? color) : 'rgba(255,255,255,0.2)',
              }}
            />
          ))}
        </div>
  ...
  return (
    <aside
      className="flex flex-col shrink-0 w-[320px] h-full border-l border-white/10 bg-gray-950/30 backdrop-blur-xl z-10"
      ...
      <div className="shrink-0 px-4 py-2.5 border-t border-white/10 flex items-center gap-1.5">
        {beatMeta.map((b, i) => (
          <button
            ...
            style={{
              backgroundColor: i === index ? (RUNG_COLOR[b.rung] ?? color) : 'rgba(255,255,255,0.12)',
