import { RefreshCw, Wifi, WifiOff, Radio, Plus, Link, Unlink, Trash2 } from 'lucide-react'
import { useLidar } from '../../context/LidarContext'
import { useVenue } from '../../context/VenueContext'
import { useTracking } from '../../context/TrackingContext'
import { LidarDevice, Vector3 } from '../../types'
  ...
  return (
    <div className="p-4 space-y-4">
        ...
        {devices.length === 0 ? (
          ...
          <div className="space-y-2">
            {devices.map(device => {
              ...
              const isPlaced = !!getPlacementByDeviceId(device.id)
              
              return (
                <div
                  key={device.id}
                  className="bg-card-bg border border-border-dark rounded-lg p-3"
                >
                  {/* Header */}
                  ...
                  <div className="flex gap-2">
                    ...
                    {!isPlaced ? (
                      <button
                        onClick={() => handleAddToScene(device)}
                        disabled={!venue}
                        className="flex-1 py-1.5 bg-highlight/20 text-highlight rounded text-xs hover:bg-highlight/30 transition-colors flex items-center justify-center gap-1 disabled:opacity-50"
                      >
                        <Plus className="w-3 h-3" />
                        Add to Scene
                      </button>
                    ) : (
                      <div className="flex-1 py-1.5 bg-gray-500/20 text-gray-400 rounded text-xs flex items-center justify-center gap-1">
                        ✓ In Scene
                      </div>
                    )}
                  </div>
      ...
      {placements.length > 0 && (
        <div>
          <h3 className="text-xs font-medium text-gray-400 uppercase tracking-wide mb-3">
            Placed in Scene ({placements.length})
          </h3>
          <div className="space-y-2">
              ...
              return (
                <div
                  ...
                  className={`bg-card-bg border rounded-lg p-3 cursor-pointer transition-colors ${
                    isSelected ? 'border-highlight' : 'border-border-dark hover:border-gray-600'
                  }`}
                >
                  {/* Header */}
                  <div className="flex items-center justify-between mb-2">
                    <div className="flex items-center gap-2">
                      <div className={`w-2 h-2 rounded-full ${device?.status === 'online' ? 'bg-green-400' : 'bg-gray-400'}`} />
                      <span className="text-sm text-white font-medium">{device?.hostname || 'Unknown'}</span>
                    </div>
                    <button
