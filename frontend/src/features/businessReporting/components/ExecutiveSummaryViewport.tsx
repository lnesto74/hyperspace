          ...
          {pillars.map(pillar => {
            ...
            return (
              ...
              >
                ...
                <div className="text-xs text-gray-400 truncate mb-0.5">{pillar.metric}</div>
                ...
                {pillar.detail && (
                  <p className="text-xs text-gray-400 mt-1 truncate">{pillar.detail}</p>
                )}
      ...
      <div className="grid grid-cols-2 xl:grid-cols-4 gap-2">
        {highlights.topZone && (
          <div className="rounded-lg border border-green-500/30 bg-green-500/5 px-3 py-2">
            <div className="text-xs text-green-400/80 mb-0.5">Best zone</div>
            <div className="text-xs text-gray-200 truncate">{highlights.topZone.name}</div>
            <div className="text-xs text-gray-400">{highlights.topZone.utilization}% engagement</div>
          </div>
        )}
        {highlights.topCategory && (
          <div className="rounded-lg border border-amber-500/30 bg-amber-500/5 px-3 py-2">
            <div className="text-xs text-amber-400/80 mb-0.5">Top category</div>
            <div className="text-xs text-gray-200 truncate">{highlights.topCategory.category}</div>
            <div className="text-xs text-gray-400">{highlights.topCategory.engagementRate}% engagement</div>
          </div>
        )}
        {highlights.topCampaign && (
          <div className="rounded-lg border border-purple-500/30 bg-purple-500/5 px-3 py-2">
            <div className="text-xs text-purple-400/80 mb-0.5">Top campaign</div>
            <div className="text-xs text-gray-200 truncate">{highlights.topCampaign.name}</div>
            <div className="text-xs text-gray-400">CES {highlights.topCampaign.ces} · EAL {highlights.topCampaign.eal}%</div>
          </div>
        )}
        {issueCount > 0 && (
          <div className="rounded-lg border border-red-500/30 bg-red-500/5 px-3 py-2">
            <div className="text-xs text-red-400/80 mb-0.5">Shelf issues</div>
            <div className="text-xs text-gray-200">{issueCount} underperforming zones</div>
            {highlights.worstZone && (
              <div className="text-xs text-gray-400 truncate">{highlights.worstZone.name}</div>
            )}
          </div>
        )}
      </div>
      ...
      {topCategories.length > 0 && (
        <div className="rounded-lg border border-gray-700/80 bg-gray-800/40 overflow-hidden">
          <div className="px-3 py-2 border-b border-gray-700/60 flex items-center justify-between">
            ...
            <span className="text-xs text-gray-400">click row → zone heatmap</span>
          </div>
          <div className="p-3">
            <CategoryVisitsPanel
