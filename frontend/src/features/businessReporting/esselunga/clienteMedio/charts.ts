...
export function columns(
  items: BarItem[],
  { unit = '', dec = 0, color = 'var(--seq-5)', h = 230, tipf }: {
...
export function hbars(
  ...
  const W = 520, L = labelw, R = 78, T = 6, rh = 26, gap = 6;
...
export function heat(
  ...
  const W = 520, L = 150, T = 24, cw = (W - L - 8) / Math.max(cols.length, 1), ch = 24;
  ...
  rows.forEach((r, i) => {
    ...
    cols.forEach((c, j) => {
      ...
      s += `<rect x="${L + cw * j + 1}" y="${T + i * ch + 1}" width="${cw - 2}" height="${ch - 2}" rx="3" fill="var(${ramp[k]})" data-tip="${esc(`${r} · ${c}: ${fmt(v, dec)}${unit}`)}"/>`
        + `<text x="${L + cw * j + cw / 2}" y="${T + i * ch + ch / 2 + 4}" text-anchor="middle" style="fill:${dark ? '#fff' : 'var(--ink)'};font-size:11.5px;pointer-events:none">${fmt(v, dec)}</text>`;
    });
  });
  return `<svg viewBox="0 0 ${W} ${H}" role="img">${s}</svg>`;
}
