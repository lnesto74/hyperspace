...
// Production track store rather than the 35-minute MQTT capture. The capture
// only ever reached a quarter of the shop floor — its coverage stopped growing
// after ~17 minutes, so the holes were the sensor's line of sight, not a
// threshold. Days of stored tracks cover 80% of the walkable area.
const field = await loadField('./field_prod.json' + bust);
...
const spanDays = (field.meta.span_hours / 24).toFixed(1);
document.getElementById('meta').innerHTML =
  `${field.NX}\u00d7${field.NY} cells at ${CELL} m \u00b7 ` +
  `${field.meta.rows_in_hours.toLocaleString()} positions over ${spanDays} days ` +
  `(${field.meta.window_local_hours[0]}\u2013${field.meta.window_local_hours[1]} local) \u00b7 ` +
  `${field.meta.steps_total.toLocaleString()} displacement steps`;

document.getElementById('planSource').textContent =
  `${layout.venue.name} \u00b7 ${layout.counts.polygons.toLocaleString()} fixtures \u00b7 `
  + `production track store, venue metres \u00b7 `
  + `${field.meta.cells_emitted.toLocaleString()} cells = 80% of walkable floor`;
